/**
 * System prompts for LLM operations, ported from pkg/llm/system.go
 */
...
export const initialPlanSystemPrompt = commonSystemPrompt + `
<testing_info>
...
export const updatePlanSystemPrompt = commonSystemPrompt + `
<testing_info>
  - The user has access to an extensive set of tools to evalulate and test your output.
  - The user will provide multiple values.yaml to test the Helm chart generation.
  - For each change, the user will run \`helm template\` with all available values.yaml and confirm that it renders into valid YAML.
...
export const detailedPlanSystemPrompt = commonSystemPrompt + `
...
export const cleanupConvertedValuesSystemPrompt = commonSystemPrompt + `
...
export const executePlanSystemPrompt = commonSystemPrompt + `
...
export const convertFileSystemPrompt = commonSystemPrompt + `
...
/**
 * Instructions for initial plan generation.
 * Ported from pkg/llm/create-knowledge.go initialPlanInstructions
 */
export const initialPlanInstructions = `
- Describe a plan for creating a new helm chart based on the user request.
- The user will provide a chart to start from. You should be inspired by this, but it's not important to copy it exactly.
- Refer the process as "creating" a chart, not "editing" a chart.
- The user is a developer who understands Helm and Kubernetes.
- You can be technical in your response, but don't write code.
- Avoid referring to the base chart in your response. For the purpose of this plan, you will describe your plan as if you are creating a new chart.
...
- After your description, output a <chartsmithArtifactPlan> block listing all files to create/update/delete.
`;

/**
 * Instructions for update plan generation.
 * Ported from pkg/llm/create-knowledge.go updatePlanInstructions
 */
export const updatePlanInstructions = `
- Describe a plan for editing an existing helm chart based on the user request.
...
/**
 * Create knowledge prompt additions.
 * Ported from pkg/llm/create-knowledge.go createKnowledge
 */
export const createKnowledge = `
- If the chart is named 'new-chart', rename it to an appropriate name. The word "replicated" is not part of the name.
